...
import { PlusIcon, Cross1Icon, ChatBubbleIcon, ChevronUpIcon, ChevronDownIcon } from "@radix-ui/react-icons"
...
const StorySection = () => {
  ...
  console.log(user.user, "userData________user11")
  // const userId = user?.state?.user?._id;
  const heartId = localStorage.getItem("userId")
  ...
  const handleLikeStory = async (storyId) => {
    ...
    try {
      const response = await fetch(`http://localhost:9003/Story/api/stories/${storyId}`, {
        method: "PUT",
        headers: {
  ...
  const fetchStories = async () => {
    try {
      const res = await axios.get("http://localhost:9003/Story/api/stories")
  ...
  const handleStoryUpload = async () => {
    ...
    try {
      await axios.post("http://localhost:9003/Story/api/stories", formData)
  ...
  const addComment = async (storyId) => {
    if (newComment.trim() && userId) {
      try {
        const response = await fetch(`http://localhost:9003/Story/api/stories/${storyId}/comments`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
    ...
    <div className="p-6 bg-gradient-to-r from-gray-50 to-blue-50 rounded-lg shadow-md">
        ...
        {stories?.map((userStory, idx) => (
          ...
          >
            <div className="w-24 h-24 bg-gray-200 rounded-full overflow-hidden shadow-inner">
              <img
                src={userStory.stories[0]?.mediaUrl || "/placeholder.svg"}
                alt={`Story ${idx + 1}`}
      ...
      {viewerModalOpen && selectedUserStory && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50">
          <div className="relative   w-[80vh] h-[80vh] bg-white rounded-xl shadow-2xl overflow-hidden flex">
            {/* Main Media Content */}
          <div className="relative flex h-full">
  {/* Image Container */}
  <div className="relative w-2/3 h-full bg-gray-100">
    <img
      src={selectedUserStory?.stories[currentStoryIndex]?.mediaUrl || "/placeholder.svg"}
      alt="story"
    ...
    <div className="absolute bottom-4 left-4 bg-black bg-opacity-50 text-white px-3 py-1 rounded-lg flex items-center space-x-2">
      <img
        src={selectedUserStory.user.profilePicture || "/placeholder.svg"}
  ...
  {/* Action Buttons - Now within the flex flow */}
  <div className="flex flex-col justify-center items-center w-1/3 p-4 space-y-4"> {/* Added w-1/3 and styling */}
    {/* Chevron Up */}
    <button
      className="p-2 bg-white bg-opacity-80 rounded-full shadow-md hover:bg-opacity-100 transition-all"
      onClick={() => setCurrentStoryIndex((prev) => Math.max(prev - 1, 0))}
    >
    ...
    <button
      className="p-2 bg-white bg-opacity-80 rounded-full shadow-md hover:bg-opacity-100 transition-all"
      onClick={() =>
        setCurrentStoryIndex((prev) => Math.min(prev + 1, selectedUserStory.stories.length - 1))
      }
    >
              ...
              <div className="w-1/3 p-4 bg-gray-50 flex flex-col">
                <div className="flex-1 overflow-y-auto">
                  <h3 className="text-base font-semibold mb-3 text-gray-800">
                    Comments (
                    {selectedUserStory?.stories?.reduce((acc, story) => acc + (story.comments?.length || 0), 0)})
                  </h3>
                        ...
                        <div key={comment._id} className="flex items-start mb-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              ...
                              <span className="text-xs text-gray-500 ml-2 cursor-pointer hover:underline">Like</span>
                              <span className="text-xs text-gray-500 ml-2 cursor-pointer hover:underline">Reply</span>
                            </p>
                          </div>
                        </div>
                      )),
