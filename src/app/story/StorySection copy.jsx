...
const StorySection = () => {
  ...
  const handleLikeStory = async (storyId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:9003/Story/api/stories/${storyId}`,
        {
          method: "PUT",
          headers: {
  ...
  const fetchStories = async () => {
    try {
      const res = await axios.get(
        "http://localhost:9003/Story/api/stories"
      );
  ...
  const addComment = async (storyId) => {
    if (newComment.trim() && userId) {
      try {
        const response = await fetch(
          `http://localhost:9003/Story/api/stories/${storyId}/comments`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            ...
            {showComments && (
              <div className="w-1/3 p-4 bg-gray-50 flex flex-col">
                <div className="flex-1 overflow-y-auto">
                  ...
                  {selectedUserStory?.stories?.some(
                    (story) => story.comments?.length > 0
                  ) ? (
