...
const ServicesDetails = () => {
  ...
  console.log(reviews, "reviews_____");
  const [refresh, setrefresh] = useState(false);

  const [likePosts, setLikePosts] = useState(new Set());
  const { fetchPost, handleLikePost, handleCommentPost, handleSharePost } =
    usePostStore();
  ...
  console.log(posts[0]?._id, "reviews_____1user");
  var idd = posts[0]?._id;
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const res = await axios.get(
          `http://localhost:9003/ReviewRoute/reviews/product/${posts[0]?._id}`
        );
        if (res.data?.status === "success") {
          setReviews(res.data.data);
          console.log(res, "reviews_____1111");
        }
      } catch (error) {
  ...
  const handleReviewSubmit = async () => {
    if (reviewText.trim()) {
      const reviewData = {
        productId: posts[0]?._id,
        listingId: user?._id, // replace this with your actual listing ID
        userName: user?.username, // get from auth or input
        reviewText: reviewText.trim(),
      };

      try {
        const response = await fetch(
          "http://localhost:9003/ReviewRoute/reviews",
          {
            method: "POST",
            headers: {
  ...
  const fetchMessages = () => {
    fetch(
      `http://localhost:9003/api/chat/messages/${posts[0].sellerId}/${user._id}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:9003/api/chat/send",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        const response = await fetch(
          `http://localhost:9003/Services/Services/${IdProduct}`
        );
        const data = await response.json();
